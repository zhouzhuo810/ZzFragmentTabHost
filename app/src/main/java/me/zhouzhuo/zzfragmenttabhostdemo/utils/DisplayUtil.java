package me.zhouzhuo.zzfragmenttabhostdemo.utils;
...
public class DisplayUtil {
    ...
    public static int scaleValue(Context context, float value) {
        ...
        if (mDisplayMetrics.scaledDensity >= DisplayConfig.SCREEN_UI_DENSITY) {
            if (width > DisplayConfig.SCREEN_WIDTH) {
                value = value * (1.3f - 1.0f / mDisplayMetrics.scaledDensity);
            } else if (width < DisplayConfig.SCREEN_WIDTH) {
                value = value * (1.0f - 1.0f / mDisplayMetrics.scaledDensity);
            }
        } else {
            float offset = DisplayConfig.SCREEN_UI_DENSITY - mDisplayMetrics.scaledDensity;
            if (offset > 0.5f) {
                value = value * 0.9f;
            } else {
                value = value * 0.95f;
    ...
    public static int scale(int displayWidth, int displayHeight, float pxValue) {
        ...
        try {
            ...
            float scaleWidth = (float) width / DisplayConfig.SCREEN_WIDTH;
            float scaleHeight = (float) height / DisplayConfig.SCREEN_HEIGHT;
